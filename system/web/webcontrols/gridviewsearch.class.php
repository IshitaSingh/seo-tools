	...
	class GridViewSearch extends GridViewControlBase
	{
		...
		public function fetchUpdateControl()
		{
			if($this->ajaxPostBack)
			{
				...
				return "'<input {$this->getAttrs()} type=\"search\" value=\"'.%{$this->dataField}%.'\" onchange=\"Rum.evalAsync(\'{$uri}/\',\'".$this->escape($params)."\',\'POST\',".\addslashes($this->ajaxStartHandler).",".\addslashes($this->ajaxCompletionHandler).");\" />'";
			}
			else
			{
