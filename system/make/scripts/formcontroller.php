<?php
	 ...
	 * @subpackage		Make
	 */
	class FormController extends MakeBase
	{
		/**
		 * Default namespace
		 * @var string
		...
		public function make($target, array $options = array())
		{
			if(isset($options[3]))
			{
				...
				if(class_exists($objectName))
				{
					...
					$controllerTemplate = str_replace("<ReturnURI>", $returnURI, $controllerTemplate);
					...
					$viewTemplate = $template = file_get_contents(\System\Base\ApplicationBase::getInstance()->config->root . "/system/make/templates/formview.tpl");
